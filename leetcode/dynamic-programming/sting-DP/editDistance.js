...
/* Top down from right to left ( i = m, j = n) */
...
function minDistance(s1, s2) {
  ...
  function solve(i, j) {
    ...
    if (s1[i] === s2[j]) {
      ...
      const insertC = solve(i, j - 1);
      const deleteC = solve(i - 1, j);
      const replaceC = solve(i - 1, j - 1);
...
/* Top down from left to right (i = 0, j = 0) */
...
function minDistance(s1, s2) {
  ...
  function solve(i, j) {
    ...
    const insertC = 1 + solve(i, j + 1);
    const deleteC = 1 + solve(i + 1, j);
    const replaceC = 1 + solve(i + 1, j + 1);
