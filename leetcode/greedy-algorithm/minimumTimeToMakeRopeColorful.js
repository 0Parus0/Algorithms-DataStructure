...
var minCost = function (colors, neededTime) {
  ...
  for (let i = 1; i < colors.length; i++) {
    if (colors[i] === colors[i - 1]) {
      ...
      // Different color, reset max for new group
...
var minCost = function (colors, neededTime) {
  ...
  while (i < n) {
    ...
    // If there are multiple balloons with same color,
    // we need to remove all but one (the one with max time)
    if (groupCount > 1) {
