...
var earliestFullBloom = function (plantTime, growTime) {
  ...
  // Create pairs of (plantTime, growTime)
  for (let i = 0; i < n; i++) {
    seeds.push([plantTime[i], growTime[i]]);
  }
  ...
  for (const [plant, grow] of seeds) {
    ...
    maxBloomDay = Math.max(maxBloomDay, totalPlantTime + grow);
  }
...
var earliestFullBloom = function (plantTime, growTime) {
  ...
  for (const i of order) {
    day += plantTime[i];
    const bloom = day + growTime[i];
    result = Math.max(result, bloom);
  }
  return result;
};
